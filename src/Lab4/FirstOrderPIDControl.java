...
// the Petri Net is being built
        ...
        int p0 = net.addPlace();
        net.setInitialMarkingForPlace(p0, FuzzyToken.zeroToken()); /// se pune cand avem un token??
        ...
        net.addArcFromPlaceToTransition(p12,t8delay,1);
        ...
        net.addArcFromTransitionToPlace(t9,p14); /// vad un cos de 0.8 unde il pun??
        ...
        net.addActionForOuputTransition(t7Out, new
                Consumer<FuzzyToken>() {
                    ...
                    public void accept(FuzzyToken t) {
                        System.out.println(
                                "Output From Transition 2: " + t.toString()); //// changed here form shortString
...
// a reference is inserted
        ...
        for (int i = 0; i < 200; i++) {
            ...
            HashMap<Integer, core.FuzzyPetriLogic.FuzzyToken> input = new HashMap<>();
            input.put(p4InpCmd,
